...
# ============================
# Hockey Overrides (Admin / HOCKEY_DATABASE_URL)
# - table: hockey_match_overrides (fixture_id BIGINT PK, patch JSONB)
# - patch.hidden == true  -> public fixtures에서 숨김 (admin merged에서는 유지)
# ============================

HOCKEY_DATABASE_URL = (os.getenv("HOCKEY_DATABASE_URL") or "").strip()
_HOCKEY_OVR_TABLE_READY = False
...
def _hockey_connect():
    ...
    if psycopg is not None:
        return psycopg.connect(HOCKEY_DATABASE_URL, row_factory=_psycopg_dict_row)
    # psycopg2 fallback
    conn = psycopg2.connect(HOCKEY_DATABASE_URL)
    return conn


def _hockey_fetch_one(sql: str, params: tuple):
    ...
    try:
        ...
        if not row:
            return None
        # psycopg3(dict_row)면 이미 dict, psycopg2면 tuple → RealDictCursor를 안썼으니 dict화
...
def _hockey_fetch_all(sql: str, params: tuple):
    conn = _hockey_connect()
    try:
        # psycopg2에서도 dict로 받기 위해 RealDictCursor 사용
        if psycopg is None:
...
def _hockey_execute(sql: str, params: tuple = ()):
    conn = _hockey_connect()
    try:
        cur = conn.cursor()
        cur.execute(sql, params)
...
def _ensure_hockey_overrides_table() -> None:
    global _HOCKEY_OVR_TABLE_READY
    if _HOCKEY_OVR_TABLE_READY:
        return
    sql = """
    ...
    """
    _hockey_execute(sql)
    _HOCKEY_OVR_TABLE_READY = True


def _load_hockey_override(fixture_id: int) -> dict:
    _ensure_hockey_overrides_table()
...
def _load_hockey_overrides_map(fixture_ids: list[int]) -> dict[int, dict]:
    if not fixture_ids:
        return {}
    _ensure_hockey_overrides_table()
...
def _hockey_status_group(status: str) -> str:
...
def _enrich_hockey_game_alias(g: dict) -> dict:
    """
    ✅ hockey fixtures 응답 구조는 유지하면서,
    ...
    if isinstance(home, dict):
        home = dict(home)
        home.setdefault("ft", home.get("score"))
        home.setdefault("ht", None)
    if isinstance(away, dict):
        away = dict(away)
        away.setdefault("ft", away.get("score"))
        away.setdefault("ht", None)

    g2 = dict(g)
    g2["fixture_id"] = g2.get("game_id")  # ✅ UI 호환 키
    g2["league_id"] = g2.get("league_id") or (league.get("id") if isinstance(league, dict) else None)
    ...
    g2["league_round"] = g2.get("stage") or g2.get("group_name")
    g2["venue_name"] = None
    g2["status_group"] = _hockey_status_group(str(g2.get("status") or ""))
    g2["elapsed"] = None
...
@app.route(f"/{ADMIN_PATH}/api/hockey/overrides/<int:fixture_id>", methods=["GET"])
@require_admin
def admin_hockey_get_override(fixture_id: int):
    patch = _load_hockey_override(fixture_id)
    return jsonify({"ok": True, "fixture_id": fixture_id, "patch": patch})


@app.route(f"/{ADMIN_PATH}/api/hockey/overrides/<int:fixture_id>", methods=["PUT"])
@require_admin
def admin_hockey_put_override(fixture_id: int):
    body = request.get_json(silent=True) or {}
    patch = body.get("patch") if isinstance(body, dict) else None
    if not isinstance(patch, dict):
        return jsonify({"ok": False, "error": "body.patch (dict) is required"}), 400
    ...
    DO UPDATE SET patch=EXCLUDED.patch, updated_at=now()
    """
    _hockey_execute(sql, (fixture_id, json.dumps(patch, ensure_ascii=False)))
    return jsonify({"ok": True})


@app.route(f"/{ADMIN_PATH}/api/hockey/overrides/<int:fixture_id>", methods=["DELETE"])
@require_admin
def admin_hockey_delete_override(fixture_id: int):
    _ensure_hockey_overrides_table()
    _hockey_execute("DELETE FROM hockey_match_overrides WHERE fixture_id=%s", (fixture_id,))
    return jsonify({"ok": True})


# ✅ Override Editor용: fixtures raw / merged (hockey)
try:
    from hockey.services.hockey_fixtures_service import hockey_get_fixtures_by_utc_range
except Exception:
    hockey_get_fixtures_by_utc_range = None  # type: ignore


@app.route(f"/{ADMIN_PATH}/api/hockey/fixtures_raw", methods=["GET"])
@require_admin
def admin_hockey_fixtures_raw():
    if hockey_get_fixtures_by_utc_range is None:
        return jsonify({"ok": False, "error": "hockey_get_fixtures_by_utc_range import failed"}), 500
    ...
    games = hockey_get_fixtures_by_utc_range(
        utc_start=utc_start, utc_end=utc_end, league_id=league_id, league_ids=league_ids
    )

    rows = [_enrich_hockey_game_alias(g) for g in games if isinstance(g, dict)]
...
def admin_hockey_fixtures_merged():
    ...
    base_rows = [_enrich_hockey_game_alias(g) for g in games if isinstance(g, dict)]

    # overrides map
    ids: list[int] = []
    for r in base_rows:
        try:
            ids.append(int(r.get("fixture_id")))
        except Exception:
            pass
    ovr_map = _load_hockey_overrides_map(ids)
    ...
    for r in base_rows:
        ...
        if patch and isinstance(patch, dict):
            ...
            rr = _deep_merge(r, p2)
            rr["_has_override"] = True
            merged_rows.append(rr)
        else:
            rr = dict(r)
            rr["_has_override"] = False
            merged_rows.append(rr)
    ...
    return jsonify({"ok": True, "rows": merged_rows, "meta": meta})
