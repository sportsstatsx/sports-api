...
def _admin_sync_header_from_timeline_patch(patch: Dict[str, Any]) -> None:
    ...
    - timeline(list)에서 GOAL/PEN_GOAL/OWN_GOAL, RED를 집계해서
      patch.header.home/away의 ft/ht/score/red_cards 를 자동 갱신한다.
    - DB 원본(matches/match_events)는 건드리지 않음(옵션1).
    """
    ...
    # ✅ 표시 레이어 동기화(FT/HT/score/red_cards)
    home["ft"] = home_ft
    home["ht"] = home_ht
    home["score"] = home_ft
    ...
    # (혹시 루트 home/away도 쓰는 곳이 있으면 같이 맞춰둠)
    if isinstance(patch.get("home"), dict):
        patch["home"]["ft"] = home_ft
        patch["home"]["ht"] = home_ht
        patch["home"]["score"] = home_ft
        patch["home"]["red_cards"] = home_red

    if isinstance(patch.get("away"), dict):
        patch["away"]["ft"] = away_ft
        patch["away"]["ht"] = away_ht
        patch["away"]["score"] = away_ft
        patch["away"]["red_cards"] = away_red
...
def admin_upsert_override(fixture_id: int):
    ...
    # ✅ 옵션1: timeline 수정 시 표시용 header(FT/HT/RED) 자동 동기화
