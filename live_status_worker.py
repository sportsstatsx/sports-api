...
def upsert_match_row_from_fixture(
    ...
    """
    matches 테이블 스키마에 맞춰 최대한 채우되, 없는 값은 NULL 허용 컬럼만 NULL로.
    returns: (fixture_id, home_id, away_id, status_group, date_utc)
    """
    ...
    fixture_id = safe_int(fx.get("id")) or 0
    date_utc = safe_text(fx.get("date")) or ""  # API-Sports 는 ISO8601 반환
    ...
    league_round = safe_text(league.get("round") or league.get("round_name"))

    elapsed = status_elapsed  # matches.elapsed는 nullable

    # matches 스키마 고정(55 rows 확인된 컬럼만 사용)
    execute(
        """
        INSERT INTO matches (
            fixture_id,
...
def upsert_match_events(fixture_id: int, events: List[Dict[str, Any]]) -> None:
    ...
    이벤트 재수집 시 중복 id는 DO NOTHING(안전) 처리.
    """
    for ev in events or []:
        ...
        a_pid = safe_int(assist.get("id"))

        ev_type = safe_text(ev.get("type")) or ""
...
def update_live_score_if_needed(fixture_id: int, status_group: str, home_goals: int, away_goals: int) -> None:
    """
    live 중에만 안전하게 덮어쓰기 (원래 로직과 동일한 가드)
    """
        ...
        for lid in league_ids:
            ...
            for item in fixtures:
                try:
                    ...
                    st = fx.get("status") or {}
                    status_short = safe_text(st.get("short")) or ""
                    status_group = map_status_group(status_short)
                    ...
                    try:
                        ...
                        if (last_ts is None) or ((now_ts - last_ts) >= STATS_INTERVAL_SEC):
