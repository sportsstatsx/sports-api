...
def _get_triple(stat: Optional[dict], key: str = "total", default: Optional[float] = None) -> Optional[float]:
    """
...
def compute_ai_predictions_from_overall(insights_overall: dict) -> Optional[dict]:
    """
    ...
    insights_overall = {
      "home": {
         "avg_gf": {"total": 1.8, "home": 2.1, "away": 1.5},
         "avg_ga": {"total": 1.2, "home": 1.0, "away": 1.4},
         "win_pct": {"total": 55, "home": 65, "away": 45},
         "draw_pct": {...},
         "over25_pct": {...},
         "team_over05_pct": {...},
         "goals_by_time_for": [ ... 10칸 ... ],
         "goals_by_time_against": [ ... 10칸 ... ],
         "first_to_score_pct": {...},
         "when_leading_win_pct": {...},
         "when_trailing_win_pct": {...},
         "yellow_per_match": 2.4,
         "red_per_match": 0.1,
         "corners_per_match": 5.6,
         ...
      },
      "away": { ... }
    }
...

    {
      ...
      "most_likely_score": {"home_goals": 2, "away_goals": 1, "pct": 18},
      "top3_scorelines": [ {...}, {...}, {...} ],
      ...
    }
    """
    ...
    h_win_hist = _get_triple(home.get("win_pct"), "total")
    h_draw_hist = _get_triple(home.get("draw_pct"), "total")
    a_win_hist = _get_triple(away.get("win_pct"), "total")
    ...
    # ───── 8) 퍼스트골 & 리드/트레일 모멘텀 ─────
    first_home_hist = _get_triple(home.get("first_to_score_pct"), "total")
    first_away_hist = _get_triple(away.get("first_to_score_pct"), "total")
    ...
    when_leading_win_home = _get_triple(home.get("when_leading_win_pct"), "total")
    when_trailing_win_home = _get_triple(home.get("when_trailing_win_pct"), "total")
